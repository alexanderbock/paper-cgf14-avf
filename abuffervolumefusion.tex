...
\new{
Using the protein scene in Figure~\ref{fig:protein} as an example, the Depth Complexity Histogram (\dch) for this camera setting is shown in \ref{fig:protein}(\subref{fig:sub:protein-dch}). 
...
All geometries and volume bounding boxes are rendered into an integer image.
The pixels of this image act as atomic counters and are incremented by the incoming fragments. 
Each pixel of the complexity image thus holds the number of fragments as depicted in Figure~\ref{fig:protein} (note that (\subref{fig:sub:protein-dci}) has been normalized and inverted for presentational purposes).  
