...
The \ab{} concept is a leading OIT solution based on the temporary storage of all intermediate fragments. 
...
Depth Complexity Histograms (\dch{}s) are histograms computed from images depicting the observed depth complexities as seen from the active viewpoint. 
Figure~\ref{fig:protein}(\subref{fig:sub:protein}, \subref{fig:sub:protein-dci}, \subref{fig:sub:protein-dch}) exemplifies a scene, its associated complexity image (inverted grayscale) and the resulting \dch. 
For the computation of a \dch, both shading and blending are first de-activated. 
...
The result is a complexity image which represents the observed depth complexity of the scene. 
The \dch{} is computed as a standard histogram over this image. 
Each bin in a \dch{} is thus associated with a complexity range and holds the number of pixels whose depth complexities fall within that range. 
...
Figure~\ref{fig:protein}(\subref{fig:sub:protein-dch}) shows a logarithmic plot of the \dch{} (the dominance of even numbered complexities is a result of closed geometry).
...
%In particular, techniques relying on \ab{}s can benefit from it since both \stencil{} and \dloop{} are compatible with most \ab{} solutions using global memory management. 
