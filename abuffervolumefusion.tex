...
\begin{figure*}[tcb]
  ...
  % 
	\hspace*{\fill}%
  \begin{minipage}[b][\boxheight][b]{0.18\linewidth}
    \centering%
...
The Depth Complexity Histogram (\dch) for the current camera setting is shown in (\subref{fig:sub:protein-dch}). 
...
Each pixel value in the complexity image thus holds the number of fragments for that pixel (note that (\subref{fig:sub:protein-dci}) has been normalized and inverted for presentational purposes).  
