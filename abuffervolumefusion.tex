...
Each pixel of the complexity image thus holds the number of fragments (note that Figure~\ref{fig:protein}(\subref{fig:sub:protein-dci}) has been normalized and inverted for presentational purposes such that white pixels represent low complexity and dark pixels represent high complexity). 
The \dch{} in Figure~\ref{fig:protein}(\subref{fig:sub:protein-dch}) is then computed as a standard histogram over this integer image. 
