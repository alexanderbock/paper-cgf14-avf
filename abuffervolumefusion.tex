...
\new{
For an example of a Depth Complexity Histogram (\dch), we use the protein scene in Figure~\ref{fig:protein}, where the DCH for the current camera setting is shown in (\subref{fig:sub:protein-dch}). 
The \dch{} is computed by first rendering a depth complexity image as shown in (\subref{fig:sub:protein-dci}). 
While rendering the complexity image, both shading and blending are disabled. 
...
Each pixel of the complexity image thus holds the number of fragments (note that (\subref{fig:sub:protein-dci}) has been normalized and inverted for presentational purposes such that white pixels represent low complexity and dark pixels represent high complexity). 
The \dch{} in (\subref{fig:sub:protein-dch}) is then computed as a standard histogram over this integer image. 
...
\minor{Note that the \dch{} in Figure~\ref{fig:protein}(\subref{fig:sub:protein-dch}) is plotted logarithmically.
